...
public sealed class PersonController(
	IValidator<PersonAddRequest> addRequestValidator,
	IValidator<PersonRequest> requestValidator,
	IEntityService<PersonApplicationModel> entityService) : ControllerBase
{
	[HttpGet("getAll")]
	public async IAsyncEnumerable<PersonResponse> GetAll([EnumeratorCancellation] CancellationToken token)
	{
	...
	public async ValueTask<IActionResult> GetById(long id, CancellationToken token)
	{
		...
		if (person is null)
		{
			return Problem("Person not found", statusCode: StatusCodes.Status404NotFound);
		}
	...
	[HttpPost("add")]
	public async Task<IActionResult> Add(PersonAddRequest request, CancellationToken token)
	{
		...
		if (!result.HasValue)
		{
			return Problem("Unable to create person", statusCode: StatusCodes.Status400BadRequest);
		}
	...
	public async Task<IActionResult> Update(long id, PersonRequest request, CancellationToken token)
	{
		if (id != request.Id)
		{
		...
		if (!updated)
		{
			return Problem("Unable to update person", statusCode: StatusCodes.Status400BadRequest);
		}
